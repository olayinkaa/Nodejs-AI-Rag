...
export default {
  async OllamaLangChainChat(req: Request, res: Response) {
    try {
      ...
      if (architecture === "graph") {
        ...
        aiReply = await InMemoryService.executeLinearChain(message);
  ...
  async OllamaRagChat(req: Request, res: Response) {
    try {
      ...
      const aiReply = await InMemoryService.executePDFLoader(message);
      res.status(200).json({ reply: aiReply });
    } catch (error: any) {
      console.log(error);
      res.status(500).json({ error: error.message });
  ...
  async streamLangChain(req: Request, res: Response): Promise<void> {
    try {
      ...
      const tokenStream =
        await InMemoryService.executeLinearChainStream(message);
